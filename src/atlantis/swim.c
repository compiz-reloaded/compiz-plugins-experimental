	   ...
	   float   speed)
{
    ...
    fish->x += speed * fish->v * cos (fish->psi / RAD) * cos (fish->theta / RAD);

    fish->y += speed * fish->v * sin (fish->psi / RAD) * cos (fish->theta / RAD);
    fish->z += speed * fish->v * sin (fish->theta / RAD);
}

void
FishMiss (AtlantisScreen *as,
	  int            i)
{
    int         j;
    float       avoid, thetal;
	...
	if (j != i)
	{
	    X = as->fish[j].x - as->fish[i].x;
	    Y = as->fish[j].y - as->fish[i].y;
	    Z = as->fish[j].z - as->fish[i].z;

	    R = sqrt (X * X + Y * Y + Z * Z);

	    avoid = 1.0;
	    thetal = as->fish[i].theta;

	    if (R < as->fish[i].size)
	    {
		if (Z > 0.0)
		{
		    as->fish[i].theta -= avoid;
		}
		else
		{
		    as->fish[i].theta += avoid;
	    ...
	    as->fish[i].dtheta += (as->fish[i].theta - thetal);
	}
    }
}
